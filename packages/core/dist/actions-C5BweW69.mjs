...
var st = (s, i, t) => i in s ? et(s, i, { enumerable: !0, configurable: !0, writable: !0, value: t }) : s[i] = t;
var u = (s, i, t) => st(s, typeof i != "symbol" ? i + "" : i, t);
const X = {
...
const it = ["enter", "leave"];
function ot(s = !1, i) {
  return s && !it.includes(i);
}
function Xt(s, i = "", t = !1) {
  const e = X[s], o = e && e[i] || i;
  return "on" + L(s) + L(o) + (ot(t, o) ? "Capture" : "");
}
...
function Wt(s) {
  ...
  const e = nt.includes(i) ? "capturecapture" : "capture", o = !!~i.indexOf(e);
  return o && (i = i.replace("capture", "")), { device: i, capture: o, passive: t };
}
function zt(s, i = "") {
  const t = X[s], e = t && t[i] || i;
  return s + e;
}
function C(s) {
  return "touches" in s;
...
function Gt(s) {
  return rt(s).map((i) => i.identifier);
}
function O(s, i) {
...
function jt(...s) {
  return s.length === 0 ? ut : s.length === 1 ? s[0] : function() {
    let i;
    for (const t of s)
...
const a = {
  toVector(s, i) {
    return s === void 0 && (s = i), Array.isArray(s) ? s : [s, s];
  },
  add(s, i) {
...
function R(s, i, t) {
  return i === 0 || Math.abs(i) === 1 / 0 ? Math.pow(s, t * 5) : s * i * t / (i + t * s);
}
function K(s, i, t, e = 0.15) {
  return e === 0 ? lt(s, i, t) : s < i ? -R(i - s, t - i, e) + i : s > t ? +R(s - t, t - i, e) + t : s;
}
function ft(s, [i, t], [e, o]) {
  const [[n, r], [c, f]] = s;
  return [K(i, n, r, e), K(t, c, f, o)];
}
const dt = 32;
class j {
  constructor(i, t, e) {
  ...
  start(i) {
    ...
    t._active || (this.reset(), this.computeInitial(), t._active = !0, t.target = i.target, t.currentTarget = i.currentTarget, t.lastOffset = e.from ? D(e.from, t) : t.offset, t.offset = t.lastOffset, t.startTime = t.timeStamp = i.timeStamp);
  }
  /**
  ...
  compute(i) {
    ...
    t.args = this.args;
    let n = 0;
    if (i && (t.event = i, e.preventDefault && i.cancelable && t.event.preventDefault(), t.type = i.type, o.touches = this.ctrl.pointerIds.size || this.ctrl.touchIds.size, o.locked = !!document.pointerLockElement, Object.assign(o, ht(i)), o.down = o.pressed = o.buttons % 2 === 1 || o.touches > 0, n = i.timeStamp - t.timeStamp, t.timeStamp = i.timeStamp, t.elapsedTime = t.timeStamp - t.startTime), t._active) {
      const p = t._delta.map(Math.abs);
      a.addTo(t._distance, p);
    }
    this.axisIntent && this.axisIntent(i);
    const [r, c] = t._movement, [f, g] = e.threshold, { _step: h, values: v } = t;
    if (e.hasCustomTransform ? (h[0] === !1 && (h[0] = Math.abs(r) >= f && v[0]), h[1] === !1 && (h[1] = Math.abs(c) >= g && v[1])) : (h[0] === !1 && (h[0] = Math.abs(r) >= f && Math.sign(r) * f), h[1] === !1 && (h[1] = Math.abs(c) >= g && Math.sign(c) * g)), t.intentional = h[0] !== !1 || h[1] !== !1, !t.intentional) return;
    const d = [0, 0];
    if (e.hasCustomTransform) {
      const [p, tt] = v;
      d[0] = h[0] !== !1 ? p - h[0] : 0, d[1] = h[1] !== !1 ? tt - h[1] : 0;
    } else
      d[0] = h[0] !== !1 ? r - h[0] : 0, d[1] = h[1] !== !1 ? c - h[1] : 0;
    this.restrictToAxis && !t._blocked && this.restrictToAxis(d);
    const E = t.offset, w = t._active && !t._blocked || t.active;
    w && (t.first = t._active && !t.active, t.last = !t._active && t.active, t.active = o[this.ingKey] = t._active, i && (t.first && ("bounds" in e && (t._bounds = D(e.bounds, t)), this.setup && this.setup()), t.movement = d, this.computeOffset()));
    const [T, A] = t.offset, [[I, k], [J, Q]] = t._bounds;
    t.overflow = [T < I ? -1 : T > k ? 1 : 0, A < J ? -1 : A > Q ? 1 : 0], t._movementBound[0] = t.overflow[0] ? t._movementBound[0] === !1 ? t._movement[0] : t._movementBound[0] : !1, t._movementBound[1] = t.overflow[1] ? t._movementBound[1] === !1 ? t._movement[1] : t._movementBound[1] : !1;
    const Z = t._active ? e.rubberband || [0, 0] : [0, 0];
    if (t.offset = ft(t._bounds, t.offset, Z), t.delta = a.sub(t.offset, E), this.computeMovement(), w && (!t.last || n > dt)) {
...
class y extends j {
  ...
  axisIntent(t) {
    ...
    if (!e.axis && t) {
      const n = typeof o.axisThreshold == "object" ? o.axisThreshold[W(t)] : o.axisThreshold;
      e.axis = pt(e._movement, n);
    }
...
class gt extends y {
  ...
  pointerUp(t) {
    ...
    const [r, c] = e._distance;
    if (e.tap = r <= o.tapsThreshold && c <= o.tapsThreshold, e.tap && o.filterTaps)
      e._force = !0;
    else {
      const [f, g] = e._delta, [h, v] = e._movement, [d, E] = o.swipe.velocity, [w, T] = o.swipe.distance, A = o.swipe.duration;
      if (e.elapsedTime < A) {
        const I = Math.abs(f / e.timeDelta), k = Math.abs(g / e.timeDelta);
        I > d && Math.abs(h) > w && (e.swipe[0] = Math.sign(f)), k > E && Math.abs(v) > T && (e.swipe[1] = Math.sign(g));
      }
    }
    this.emit();
  }
  ...
  setupScrollPrevention(t) {
    this.state._preventScroll = !1, _t(t);
...
function vt() {
  return $() || b && window.navigator.maxTouchPoints > 1;
}
function yt() {
...
const l = {
  ...
  pointer: yt(),
  pointerLock: bt()
}, wt = 250, Tt = 180, At = 0.5, Dt = 50, It = 250, kt = 10, H = { mouse: 0, touch: 0, pen: 8 }, q = {
  ...m,
  device(s, i, { pointer: { touch: t = !1, lock: e = !1, mouse: o = !1 } = {} }) {
    return this.pointerLock = e && l.pointerLock, l.touch && t ? "touch" : this.pointerLock ? "mouse" : l.pointer && !o ? "pointer" : l.touch ? "touch" : "mouse";
  },
  preventScrollAxis(s, i, { preventScroll: t }) {
    if (this.preventScrollDelay = typeof t == "number" ? t : t || t === void 0 && s ? wt : void 0, !(!l.touchscreen || t === !1))
  ...
  swipe({ velocity: s = At, distance: i = Dt, duration: t = It } = {}) {
    return {
...
class Ct extends j {
  ...
  axisIntent() {
    ...
    if (!t.axis) {
      const n = Math.abs(e) * xt - Math.abs(o);
  ...
  pointerStart(t) {
    ...
    if (e._active && Array.from(o.keys()).every((c) => n.has(c)) || (o.size < 2 && o.set(t.pointerId, t), e._pointerEvents.size < 2)) return;
    this.start(t);
  ...
  pinchMove(t, e) {
    ...
    let c = 0;
    Math.abs(r) > 270 && (c += Math.sign(r)), this.computeValues([e.distance, e.angle - 360 * c]), o.origin = e.origin, o.turns = c, o._movement = [o._values[0] / o._initial[0] - 1, o._values[1] - o._initial[1]], this.compute(t), this.emit();
  }
  touchEnd(t) {
  ...
  wheelChange(t) {
    ...
    const o = this.state;
    o._delta = [-G(t)[1] / St * o.offset[0], 0], a.addTo(o._movement, o._delta), F(o), this.state.origin = [t.clientX, t.clientY], this.compute(t), this.emit();
  }
  wheelEnd() {
...
const Mt = {
  ...
  bounds(s, i, { scaleBounds: t = {}, angleBounds: e = {} }) {
    const o = (r) => {
      const c = V(D(t, r), { min: -1 / 0, max: 1 / 0 });
      return [c.min, c.max];
    }, n = (r) => {
      const c = V(D(e, r), { min: -1 / 0, max: 1 / 0 });
      return [c.min, c.max];
    };
...
class Vt extends y {
  ...
  wheelChange(t) {
    const e = this.state;
    e._delta = G(t), a.addTo(e._movement, e._delta), F(e), this.compute(t), this.emit();
  }
  wheelEnd() {
    this.state._active && (this.state._active = !1, this.compute(), this.emit());
  }
...
const Ut = {
  ...m,
  mouseOnly: (s = !0) => s
}, Bt = /* @__PURE__ */ new Map(), Yt = /* @__PURE__ */ new Map();
function $t(s) {
  Bt.set(s.key, s.engine), Yt.set(s.key, s.resolver);
}
const qt = {
  key: "drag",
  engine: gt,
